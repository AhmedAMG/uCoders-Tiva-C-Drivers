...
void TIMER_Init(GPTM_MODULE module, GPTM_SUBMODULE sub_module, GPTM_MODE mode, GPTM_DIRECTION direction, GPTM_STALL stall){
    uint8_t AB = sub_module%8; //A=0,B=4, AB = A
    uint8_t concatenation = sub_module/8;
...
//Enable the interrupt source from the 5 options and register ISR pointer
...
uint8_t FANCYTIMER_ONESHOT_SetTimeoutSeconds(GPTM_MODULE module, GPTM_SUBMODULE sub_module, uint32_t msec, void (*function) (void)){
    ...
    if(interval_load > 0xFFFF){
        if(sub_module != TIM_AB){return 0;}
    }

    TIMER_Init(module, sub_module, TIMER_MODE_ONE_SHOT, COUNT_UP, STALL_ENABLE);

    TIMER_SetIntervalLoad(module, sub_module, (uint32_t)interval_load);
    TIMER_EnableAndSetIntISR(module, sub_module, INT_TIMEOUT, function);
    return 1;
}
...
uint8_t FANCYTIMER_ONESHOT_SetMatchSeconds(GPTM_MODULE module, GPTM_SUBMODULE sub_module, uint32_t msec, void (*function) (void)){
    ...
    TIMER_SetMatch(module, sub_module, (uint32_t)match);
    TIMER_EnableAndSetIntISR(module, sub_module, INT_NON_CAPTURE_MATCH, function);
    return 1;
}
...
uint8_t FANCYTIMER_PERIODIC_SetTimeoutSeconds(GPTM_MODULE module, GPTM_SUBMODULE sub_module, uint32_t msec, void (*function) (void)){
    ...
    TIMER_Init(module, sub_module, TIMER_MODE_PERIODIC, COUNT_UP, STALL_ENABLE);
...
uint8_t FANCYTIMER_ECOUNT_SetEventsNumber(GPTM_MODULE module, GPTM_SUBMODULE sub_module, uint32_t events, GPTM_EDGE edge, GPTM_CONTINUITY continuity, void (*function) (void)){
    ...
    if(continuity == EDGECOUNT_CONTINUOUS){
        TIMER_Init(module, sub_module, TIMER_MODE_INPUT_EDGE_COUNT, COUNT_UP, STALL_ENABLE);
        TIMER_SetIntervalLoad(module, sub_module, 0x0000FFFF);
        TIMER_SetMatch(module, sub_module, events);
        TIMER_SetPrescalar(module, sub_module, 0xFF);
        if(events > 0xFFFF) TIMER_SetPrescalerMatch(module, sub_module, events>>16);
    }else{
        TIMER_Init(module, sub_module, TIMER_MODE_INPUT_EDGE_COUNT, COUNT_DOWN, STALL_ENABLE);
        TIMER_SetIntervalLoad(module, sub_module, 0x0000FFFF);
        TIMER_SetMatch(module, sub_module, 0x0000FFFF - events);
        if(events > 0xFFFF){
            TIMER_SetPrescalar(module, sub_module, 0xFF);
            TIMER_SetPrescalerMatch(module, sub_module, 0xFF - (events>>16));
        }

    }

    TIMER_ConfigEdgeDetection(module, sub_module, edge);
    TIMER_EnableAndSetIntISR(module, sub_module, INT_CAPTURE_MATCH, function);
    TIMER_EnableSnapshot(module, sub_module);
    return 1;
}
...
uint8_t FANCYTIMER_ETIME_Configure(GPTM_MODULE module, GPTM_SUBMODULE sub_module, GPTM_EDGE edge, void (*function) (void)){
    ...
    TIMER_Init(module, sub_module, TIMER_MODE_INPUT_EDGE_TIME, COUNT_UP, STALL_ENABLE);
    TIMER_ConfigEdgeDetection(module, sub_module, edge);
    TIMER_SetIntervalLoad(module, sub_module, 0x0000FFFF);
    TIMER_SetPrescalar(module, sub_module, 0xFF);
    TIMER_EnableAndSetIntISR(module, sub_module, INT_CAPTURE_EVENT, function);
    TIMER_EnableSnapshot(module, sub_module);
    return 1;

}

uint8_t FANCYTIMER_PWM_SetDutyCycle(GPTM_MODULE module, GPTM_SUBMODULE sub_module, float freq_Hz, uint8_t duty, GPTM_CCP ccp, void (*function) (void)){
    if(sub_module == TIM_AB) return 0;
    ...
    TIMER_Init(module, sub_module, TIMER_MODE_PWM, COUNT_DOWN, STALL_ENABLE);
    TIMER_SetIntervalLoad(module, sub_module, i);
    TIMER_SetPrescalar(module, sub_module, i>>16);
    i = (i*(duty/100.0));
    TIMER_SetMatch(module, sub_module, i);
    TIMER_SetPrescalerMatch(module, sub_module, i>>16);
    TIMER_ConfigCCP(module, sub_module, ccp);

    if(function != 0) TIMER_EnableAndSetIntISR(module, sub_module, INT_CAPTURE_EVENT, function);
    TIMER_EnableSnapshot(module, sub_module);
    return 1;
}
...
void TIMER0A_ISR(void){
    ...
    volatile uint32_t *reg_icr = (volatile uint32_t *)(TIM0 + (TIMERICR));
    volatile uint32_t *reg_mis = (volatile uint32_t *)(TIM0 + (TIMERMIS));
...
void TIMER2A_ISR(void){
    uint16_t i = 1<<0;
    uint8_t bit_position = 0;
    volatile uint32_t *reg_icr = (volatile uint32_t *)(TIM2 + (TIMERICR));
    volatile uint32_t *reg_mis = (volatile uint32_t *)(TIM2 + (TIMERMIS));
...
void TIMER5A_ISR(void){
    uint16_t i = 1<<0;
    uint8_t bit_position = 0;
    volatile uint32_t *reg_icr = (volatile uint32_t *)(TIM5 + (TIMERICR));
    volatile uint32_t *reg_mis = (volatile uint32_t *)(TIM5 + (TIMERMIS));
