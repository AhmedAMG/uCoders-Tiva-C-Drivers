...
int main(void)
{
    ...
    //FANCYTIMER_ONESHOT_SetTimeoutSeconds(TIM2, TIM_AB, 1000, TIMER_Handler);
    //FANCYTIMER_ONESHOT_SetMatchSeconds(TIM2, TIM_AB, 500, TIMER_Handler);
    //FANCYTIMER_PERIODIC_SetTimeoutSeconds(TIM2, TIM_AB, 1000, TIMER_Handler);
    //FANCYTIMER_PERIODIC_SetMatchSeconds(TIM2, TIM_AB, 500, TIMER_Handler);
    //FANCYTIMER_ECOUNT_SetEventsNumber(TIM2, TIM_A, 5, EDGE_FALLING, EDGECOUNT_CONTINUOUS, TIMER_Handler);
    FANCYTIMER_ETIME_Configure(TIM2, TIM_A, EDGE_FALLING, TIMER2_Handler);
    FANCYTIMER_PWM_SetDutyCycle(TIM0, TIM_B, 1, 50, CCP_INVERTED, TIMER0_Handler);

    NVIC_SetPriority(23, 3);
    NVIC_SetPriority(20, 5);
    TIMER_Start(TIM2, TIM_A);
    TIMER_Start(TIM0, TIM_B);
