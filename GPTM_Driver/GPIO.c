...
static void (*(PORTF_interrupt_bits[8])) (void);
static void (*(PORTE_interrupt_bits[8])) (void);
...
void GPIO_SetIntHandler(GPIO_PORT port, uint8_t pin, void (*function) (void)){
    ...
    switch(port){
    case PORTF:
        PORTF_interrupt_bits[bit_position] = function;
        break;
    case PORTE:
        PORTE_interrupt_bits[bit_position] = function;
        break;
    }
...
void GPIOF_ISR(void){
    ...
    reg_iev = reg_iev + BusSelection[5]*0xE000;
    uint32_t data_mis = *reg_mis;
    uint32_t data_iev = *reg_iev;
    ...
    if(count>900){
        PORTF_interrupt_bits[bit_position]();
    }
...
void GPIOE_ISR(void){
    ...
    reg_mis = reg_mis + BusSelection[4]*0xE000;
    reg_iev = reg_iev + BusSelection[4]*0xE000;
    uint32_t data_mis = *reg_mis;
    uint32_t data_iev = *reg_iev;
    ...
    for(temp = 0; temp < 1000; temp++){
        if((data_iev & i) == i){
            if((GPIORead(PORTE, (i))&(i)) == i){
                count++;
            }
        }else{
            if((GPIORead(PORTE, (i))&(i)) == 0){
                count++;
            }
        }
    }
    ...
    if(count>900){
        PORTE_interrupt_bits[bit_position]();
    }

    reg_icr = reg_icr + BusSelection[4]*0xE000;
    uint32_t data_icr = *reg_icr;
    data_icr |= (i);
    *reg_icr = data_icr;
